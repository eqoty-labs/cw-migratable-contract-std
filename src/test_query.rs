...
mod tests {
    ...
    use crate::state::{MigratedFromState, MigratedToState, MIGRATED_FROM, MIGRATED_TO};
    ...
    fn query_migrated_info_returns_saved_migrated_from_contract() -> StdResult<()> {
        ...
        assert_eq!(Some(saved_migrated_from.contract), queried_migrated_from);
        Ok(())
    }
    ...
    fn query_migrated_info_returns_saved_migrated_to_contract() -> StdResult<()> {
        ...
        assert_eq!(Some(saved_migrated_to.contract), queried_migrated_to);
        Ok(())
    }
}
